...
reportPeriodText <- 
  paste(format(lubridate::today() - 6, "%d %B"), "to", format(lubridate::today(), "%d %B %Y"))
```

# 1.0 Introduction
This is the NESREA Web Monitoring Report for the period **`r reportPeriodText`**[^1].
...
thisMth <- format(lubridate::today(), "%B")
thisYr <- format(lubridate::today(), "%Y")

mth_news <- newsData %>%
  dplyr::filter(Month == thisMth & Year == thisYr)
wk_news <- newsData %>%
  dplyr::filter(as.Date(Date) >= lubridate::today() - 7)
```
...
all_data <- params$data$tweets
all_data <- process_stored_tweets(all_data)
...
wk_data <- all_data %>%
  dplyr::filter(date_only >= (lubridate::today() - 6) & date_only <= (lubridate::today())) %>%
  dplyr::arrange(date_only) %>%
  dplyr::mutate(day = weekdays(date_only, abbreviate = TRUE)) %>%
  dplyr::mutate(Type = ifelse(isRetweet, "Retweet", "Original"))
wk_data$day <-  factor(wk_data$day,
                       levels = unique(wk_data$day),
                       ordered = TRUE)
  
last_wk <- all_data %>%
  dplyr::filter(date_only >= (lubridate::today() - 13) & date_only <= (lubridate::today() - 7))
...
no.wk <- nrow(wk_data)
month_begin <- lubridate::floor_date(lubridate::today(), "month")
month_end <- lubridate::ceiling_date(lubridate::today(), "month")
mth_data <- dplyr::filter(all_data, date_only >= month_begin & date_only <= month_end)
mostRTed <- wk_data$text[which.max(wk_data$retweetCount)]
mostFaved <- wk_data$text[which.max(wk_data$favoriteCount)]
tweets_by_us <- dplyr::filter(wk_data, screenName == "NESREANigeria")
busiest_day <- which.max(table(wk_data$date_only))
busiest_day <- lubridate::ymd(names(busiest_day))
```

|     **Description**                      |    **Result**                  |
|------------------------------------------|--------------------------------|
|No. of tweets in `r format(lubridate::today(), "%B")`|`r nrow(mth_data)`              |
...
fbPosts <- params$data$fbposts %>%
  prepare_data(.) %>%
  dplyr::select(message:shares_count) %>%
  dplyr::mutate(created_mth = format(as.Date(created_time), "%B")) %>%
  dplyr::mutate(created_yr = format(as.Date(created_time), "%Y"))
...
mth_Posts <- fbPosts %>%
  dplyr::filter(created_mth == thisMth & created_yr == thisYr)
wk_Posts <- mth_Posts %>%
  dplyr::filter(created_time >= (lubridate::today() - 6))
```
...
ggplot2::ggplot(wk_data, ggplot2::aes(created)) +
  ggplot2::geom_density(aes(fill = Type), alpha = .5) +
  ggplot2::theme(legend.justification = c(1, 1), legend.position = c(1, 1)) +
  ggplot2::ggtitle(paste("Distribution of tweets for", reportPeriodText)) +
  ggplot2::xlab("Date")
```
...
ggplot2::ggplot(fbPosts, ggplot2::aes(created_time)) +
  ggplot2::geom_density(fill = "purple", alpha = 0.6) +
  ggplot2::ggtitle("Distribution of Facebook posts")
```
