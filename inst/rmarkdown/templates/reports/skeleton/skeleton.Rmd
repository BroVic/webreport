...
```{r web time-series}
useBase <- FALSE
make_ts(data = data$webnews, platform = 'website', base = useBase)
```

```{r tweet time-series}
make_ts(data = data$tweets, platform = 'twitter', base = useBase)
## TODO: Implement month & year transitions
```

```{r fb time-series}
make_ts(data = data$fbposts, platform = 'facebook', base = useBase)
```
