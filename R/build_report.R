...
build_webreport <- function(data.source, outfile = NULL, launch.file = TRUE, ...)
{
  ...
  if (!grepl('.db$|.sqlite$', data.source, ignore.case = TRUE)) {
    stop("The file 'data.source' should have '.db' or '.sqlite' extension.")
    # TODO: Use binary file validation
  }

  ## Validation or creation of output filename
  fext <- ".docx"
  if (is.null(outfile)) {
    outfile <-
    ...
    if (!endsWith(tolower(outfile), fext))
      outfile <- paste0(outfile, fext)

      ## Check for...
      anyDuplicDots <- "\\.{2,}"
      if (grepl(anyDuplicDots, outfile))
        outfile <- gsub(anyDuplicDots, ".", outfile)
  }
  template <- system.file("rmarkdown/templates/reports/skeleton/skeleton.Rmd",
                     package = "webreport",
                     mustWork = TRUE)
